...
use super::message::*;
use super::{Error, Result, VhostUserMasterReqHandler};
use std::mem;
...
impl VhostUserMasterReqHandler for SlaveFsCacheReq {
    ...
    fn fs_slave_map(&mut self, fs: &VhostUserFSSlaveMsg, fd: RawFd) -> Result<()> {
        self.send_message(SlaveReq::FS_MAP, fs, Some(&[fd]))
    }

    /// Handle virtio-fs unmap file requests from the slave.
    fn fs_slave_unmap(&mut self, fs: &VhostUserFSSlaveMsg) -> Result<()> {
